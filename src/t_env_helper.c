...
void	t_env_add_variable(t_env *env, char *key, char *value)
{
	...
	if (t_hash_get(env->hash, key))
	{
		...
		ft_strdel(&tmp);
		tmp = ft_fstrmcat(ft_strmcat(key, "="), value);
		env->list = ft_strlist_add(env->list, tmp);
		ft_strdel(&tmp);
		t_hash_update(env->hash, key, ft_strdup(value));
	}
	else
	{
		tmp = ft_fstrmcat(ft_strmcat(key, "="), value);
		env->list = ft_strlist_add(env->list, tmp);
		ft_strdel(&tmp);
		t_hash_add(env->hash, ft_strdup(key), ft_strdup(value));
	}
}
